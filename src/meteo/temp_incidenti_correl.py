...
weather_index = 'Humidity'

# Per ogni anno in cui si ha a disposizione il campo mese, si calcola la correlazione tra i
# fattori meteo e gli incidenti 
for year in [2010, 2011, 2012, 2013]: 
    meteo_2010 = mgu.get_weather_by_year(meteo, 2013)

    meteo_per_mese = pd.Series() 
    for month in range(1,13): 
        media = mgu.get_weather_by_month(meteo_2010, month)[weather_index].mean()
