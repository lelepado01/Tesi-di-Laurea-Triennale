...
\begin{itemize}
    \item Uso di una \textbf{Licenza Aperta};
...
Va osservato, che la categorizzazione tramite le cinque stelle di Berners-Lee, lascia 
svariati problemi, come l'assenza di vincoli sulla frequenza di aggiornamento, o 
sulla granularità dei dati. 
Queste categorie iniziali hanno permesso, tuttavia, una prima valutazione 
...
\todo{lelepado: questo filepath quando lo visualizzo in pdf esce dai margini, se lo tengo in  linea come testo}
...
\end{figure}
\todo{Nell'ultima call mi sono dimenticato di dirle che ho modificato questa mappa, prima era calcolata sovrapponendo i punti e facendo uso della trasparenza, adesso ho invece approssimato le coordinate di ogni punto per avere effetto bubble chart}
...
\end{code}
\todo{lelepado: che lei sappia, non c'è un modo per aumentare il margine dello sfondo grigio dietro il codice? Ho cercato ma tutti i comandi non  modificano il limite del colore, che rimane sempre 'attaccato' al codice}
...
\todo{lelepado: nel link che mi ha mandato non ho visto nulla per gli orari, vanno bene così o devo usare il punto, come 7.00?}
...
\end{figure}
\todo{lelepado: se il codice che ho usato per questo tipo di heatmap è stato in parte preso dalla documentazione/esempi, devo citarlo?}
...
\todo{controllare se i label escano dal margine}
...
\todo{lelepado: questi due label su pdf escono dai margini}
