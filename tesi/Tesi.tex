...
dall'Istituto nazionale di statistica, in particolare dall'archivio\footnote{\url{https://www.istat.it/it/archivio/87539}}
...
dati riferiti alle linee ATM, scaricati sul sito del comune di Milano\footnote{\url{https://dati.comune.milano.it/dataset/ds532-atm-composizione-percorsi-linee-di-superficie-urbane}}.
Dallo stesso sito proviene anche il dataset riguardante gli autobus turistici, che 
contiene in particolare le aree di sosta di questi ultimi\footnote{\url{https://dati.comune.milano.it/dataset/ds740_sosta_bus_gt_turistici}}.
...
solamente le linee tranviarie, trovato sul geoportale del comune di Milano\footnote{\url{https://geoportale.comune.milano.it/ATOM/SIT/DBT2012/DBT2012_STRATO_01_Dataset_1.xml}}.
...
patentati per regione, trovato nel sito del Ministero dei 
...
\todo{lelepado: va giustificato? atrent: cosa? lelepado: il decremento di incidenti per covid}
...
figura \ref{fig:autovelox-indici}\todo{NEW}.
...
La mappa \ref{fig:autovelox-incidenti}\todo{NEW} è stata realizzata contando gli incidenti avvenuti 
...
\end{figure}
\todo{prova ad affiancare i grafi invece che uno sopra l'altro}
...
\todo{lelepado: va bene se uso italico per indicare i campi di un dataset?}
...
Ciò che è possibile ricavare dalla figura \ref{fig:rapporto-tipologie} \todo{NEW}, è che non 
...
\todo{controllare congruenza dei risultati}
...
\todo{lelepado: altro? chiudere così non mi piace}
...
Nella figura \ref{fig:numero-feriti}\todo{NEW} si deduce che esistono tipologie di sinistri che 
...
particolarmente distante dagli altri dati disponibili \cite{PROB.E.STATISTICA:1}}
come in figura \ref{fig:pedoni-no-rett}\todo{NEW}, il grafo ottenuto è più bilanciato, 
...
Per prima cosa si sono utilizzati i campi \textit{età\_pedone} per creare un grafo in cui sia 
...
Se si normalizza per anni contenuti in ogni fascia, ipotizzando un numero costante di persone di ogni 
età\footnote{La normalizzazione tiene conto che la fascia di età \textit{65+} vale 
venti anni.}, 
...
\todo{aggiungi a dati mancanti il numero di incroci / strade a milano o in altre province}
...
si è calcolata la deviazione standard per ogni regione
\footnote{Deviazione standard: misura della distanza dei dati di un campione dalla 
media\cite{PROB.E.STATISTICA:2}}.
Risulta che la regione con maggiore cambiamento di incidenti di anno in anno è il Lazio, 
con $324$ incidenti l'anno, 
\todo{lelepado: è giusto dire questo?}
mentre quella con minore variazione è la Valle d'Aosta, con $8$.
...
Il grafo \ref{fig:incidenti-estate-inverno} mostra che, per alcune regioni, 
la differenza del  numero di incidenti tra Agosto e Gennaio è molto ampia, 
come per Trentino, Emilia, Veneto e Friuli.
Tuttavia, sembra che le regioni che subiscono questo incremento di incidenti in 
Agosto, siano omogeneamente sparse, e non solo nel sud Italia.

Un modo per approfondire questo argomento, potrebbe essere l'utilizzare tutti i mesi estivi e 
...
Nella figura \ref{fig:estate-inverno}\todo{NEW}, sono rappresentati i numeri di incidenti presenti in mesi 
invernali e estivi, divisi per regione.
...
In conclusione, il comportamento che si stava cercando, cioè più incidenti in estate nel 
sud Italia, non è visibile. 
Un modo in cui potrebbe essere possibile ricavarlo, però, è osservare gli incidenti 
sulle autostrade principali.
...
\end{figure}\todo{errore label 2011 invece di 2013}

La figura \ref{fig:incidenti-temp}\todo{NEW} mostra, partendo dall'alto, il numero di incidenti 
...
\todo{atrent: esatto, va esplicitato bene che è un'analisi che lascia il tempo che trova}
