...
dall'Istituto nazionale di statistica, in particolare dall'archivio\footnote{\url{https://https://www.istat.it/it/archivio/87539}}\todo{atrent: non va lo spazio tra la parola e la footnote} 
...
L'ultimo dataset, trovato dopo veloce cosultazione\todo{attenzione allo spelling!} con un dipendente Istat, è stato il dataset ACI
...
tuttavia questo consisteva in un file molto grande e difficile da elaborare.\todo{atrent: io non lo direi, non c'è file che non si possa elaborare...}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Scopo del lavoro}\todo{atrent: ah ecco! questa parte va in cima}
...
rosse, arancioni e verdi, si è optato per la ricerca di un altro dataset.\todo{atrent: terminologia poco tecnica...}
...
\end{figure}\todo{atrent: ma questi grafici li hai generati tu?}
...
\end{lstlisting}\todo{atrent: non trovo questo snippet nei sorgenti, come mai?}
...
\end{figure}\todo{atrent: vale un po' ovunque, attenzione alle unità di misura e alle scale degli assi}
...
\begin{figure}
    ...
    \label{fig:22-marzo}
\end{figure}\todo{atrent: conclusione traibile solo a occhio o sei riuscito a dare un valore numerico?}
...
\todo{atrent: vale in generale, manca bibliografia scientifica, ti dovresti confrontare con un po' di accademia sugli studi di incidentalità, come vengono calcolati i vari fattori?}
...
\todo{lelepado: aggiungere traffico con immagini googlemaps?}
\todo{ci sono vie con la tendenza opposta?}

%\clearpage
\subsection{Correlazione tra pavè e incidentalità}\todo{atrent: vale ovunque tu parli di ``correlazione'', il termine è tecnico e se lo usi devi poi dare dei valori di correlazione (appunto) statistica, cioè attenzione a dire in generale ``è VISIBILE la correlazione tra...'', il ``visibile'' ti può aiutare a concentrarti su un subset di dati, ma poi devi dare dei numeri}
...
\begin{figure}
    \includegraphics[width=0.42\linewidth]{../src/tram/tram.png}
    \includegraphics[width=0.58\linewidth]{../dataset/pave/Cartina_Milano_Strade_Pavimentazione_Pavé_2.jpg}
...
%\clearpage
\subsection{Il sesso dei conducenti cambia in base alla strada?}\todo{atrent: rephrase, così sembra umoristico ;)}
...
Correlazione tra percentuale traffico e incidenti: $0.905$\todo{atrent: ecco, qui giusto, vale sempre, esplicitare il fattore di correlazione e magari anche la query e i dataset esatti usati per calcolarla, in modo che sia riproducibile da altri}
