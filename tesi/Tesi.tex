...
soprattutto negli anni precedenti al 2017 quando, per ottenere le loro informazioni sul 
meteo si sarebbe dovuto compilare un 
...
\end{figure}
\footnotetext{fonte immagine Form Arpa: Trentini}
...
\lighttext{(\\
    ...
    \indent localizzazione\_incidente\footnote{zona incidente come urbana, extraurbana o autostrada}\\
    ...
    \indent veicolo\_\_a\_\_\_et\_\_conducente\footnote{questo e i due campi successivi sono ripetuti anche per i passeggeri},\\
    \indent veicolo\_\_a\_\_\_sesso\_conducente,\\
    \indent veicolo\_\_a\_\_\_esito\_conducente, \\
    \indent morti\_entro\_24\_ore,\\
    \indent morti\_entro\_30\_giorni\footnote{questo insieme non contiene la categoria precedentemente},\\
    \indent feriti, Ora, trimestre\footnote{prima del 2014 questo campo è sostituito da 'anno'}\\
)}
...
incidenti avvenuti nella città nel 2016, con la relativa 
posizione\footnote{La fonte è la stessa del dataset precedente, ma i proprietari dell'articolo sono 
riusciti a fare pubblicare da Istat una parte delle posizioni geolocalizzate di incidenti a Milano, 
normalmente private per proteggere la privacy delle persone coinvolte.}.
...
il numero di accessi corrisponda, almeno in piccola parte, al volume di traffico di Milano.
...
con più incidenti a Milano, per quanto il risultato ottenuto non sarà mai troppo preciso, 
...
contenenti informazioni come il numero identificativo della linea, 
...
solamente le linee tranviarie, trovato sul portale di dati geolocalizzati 
del comune di 
...
gli incidenti a Milano siano distribuiti con concentrazioni più alte, 
...
e la zona dei Navigli}, e Piazza Kennedy e Cavalcavia del Ghisallo. 

La mappa destra nella figura \ref{fig:heatmap-municipi} è stata realizzata 
approssimando ogni coordinata di incidente. 
Normalmente queste hanno precisione di sette cifre decimali, arrotondando a cinque e 
contando i punti uguali, è stato possibile aggregare sinistri che avvengono in zone 
vicine. 
Questi dati elaborati sono poi stati memorizzati nel file reperibile seguendo il 
...
ottenuti con il numero di incidenti in zone più ristrette come, per esempio, 
...
Una domanda che sorge spontanea è, perché nel centro storico risulta tra le aree con maggiore 
...
Un esempio di ciò è quello di zona Navigli, in figura \ref{fig:navigli}, dove le vie 
interessate sono Viale Gian Galeazzo e Viale Beatrice D'Este, 
...
Google Maps, tuttavia non offrono una stima ideale, poiché indicano il traffico in tempo reale e 
...
cioè gli incidenti per chilometro per ogni municipio di Milano. 
...
di marcia, dove sono parcheggiate automobili su entrambi i lati, e la velocità dei 
...
un tracciato con la maggior parte delle strade in pavé a Milano.
...
\'E possibile notare alcune somiglianze, come tra viale Sabotino e via 
...
Una volta creata l'area delle strade in pavé, è possibile calcolare quanti sono gli 
...
Il numero di incidenti per chilometro quadrato, nelle strade in pavé è $229.45$, che 
...
quelle in pavé, sia per zona, sia per topologia. 
Inoltre, per lo stesso motivo, l'area totale dei due dataset è simile.

Eseguendo lo stesso calcolo realizzato per le strade in pavé, risulta che il numero di incidenti per 
...
circonvallazione interna e altre zone molto trafficate, mentre il campione di strade in pavé 
...
Una volta trovati gli incidenti totali, è possibile calcolare il numero di sinistri 
...
Questa ultima colonna, in particolare, è divisa per 3.14 perché l'area del cerchio 
...
Viale Monte Ceneri, il volume di incidenti è particolarmente più alto di 
...
In altre parole, la percentuale, per esempio, di autotreni sulle autostrade, è diversa rispetto 
...
urbane, mentre il numero si alza sulle autostrade.
...
Cercando nell'archivio, si è concluso che il rapporto del 2010 non sia più reperibile, 
tuttavia sono disponibili quelli più recenti, in particolare per 
...
Uno di questi fattori, che distrarrebbe il conducente, potrebbe essere l'utilizzo 
...
rispetto al resto d'Italia, e il flusso di pendolari, alla guida in orari precedenti 
...
Il risultato di questo procedimento è raffigurato nel grafo \ref{fig:rapp-inc-traff}.
...
stimato\footnote{Si è utilizzato l'anno 2012 perché è il primo anno contenente tutti i dati completi, 
...
Per eseguire questo confronto sono state prese in considerazione le località di Rimini e Palermo, 
...
degli incidenti.
Nonostante ciò, assumendo che Gennaio 2010 sia un outlier, il numero di incidenti in 
...
per trimestre dove, per primo trimestre si 
...
altre località raffigurate, avviene la tendenza opposta poiché 
...
Prima di fare assunzioni errate, va detto che questo grafo, senza un contesto, 
non è molto utile, perché il numero di incidenti di una categoria deve 
...
Nella terza categoria, d'altra parte, si iniziano ad osservare delle differenze, 
poiché si alternano \columnstyle{investimento di pedoni} e \columnstyle{scontro laterale}, 
...
Si arriva a questa conclusione perché la colonna di questo tipo di incidente, è tra 
...
\columnstyle{30-44} contiene quindici anni, mentre 
...
D'altra parte, l'indice di mortalità non cambia, poiché è realizzato tramite 
...
nelle strade provinciali e sulle autostrade, nell'anno 2018.
...
Come già accennato in precedenza, una delle motivazioni principali 
...
le strade più pericolose, concentrandosi sulla città di Milano. 
...
e controllare se esistano zone che alzano il volume di incidenti, poiché 
...
Ovviamente ci si aspetta che la risposta abbia un esito positivo, poiché 
...
\footnotetext[9]{\url{https://www.ilmeteo.it/portale/archivio-meteo/Milano/2010/}}
\footnotetext[10]{\url{https://zenodo.org/record/3992354}}
\footnotetext[11]{\url{https://geoportale.comune.milano.it/ATOM/SIT/Municipi/Municipi_Dataset_1.xml}}
\footnotetext[12]{\url{https://www.mit.gov.it/sites/default/files/media/notizia/2017-07/INFOGRAFICA Dati sintesi patenti Italia.pdf}}
\footnotetext[13]{\url{https://geoportale.comune.milano.it/ATOM/SIT/DBT2012/DBT2012_STRATO_01_Dataset_1.xml}}
\footnotetext[14]{\url{https://github.com/openpolis/geojson-italy}}
\footnotetext[15]{\url{https://www.istat.it/it/archivio/16777}}
