...
atrent: certamente, ma non in intro, farei un'appendice col codice commentato/spiegato}
...
spediti via email, dunque con un peso massimo di 20Kb.\todo{MB}
Infine, per evitare che persone creassero script per automatizzare le richieste, 
era presente un captcha\todo{spiega in footnote, e il termine va in grassetto}, in modo da richiedere l'intervento di una persona 
reale per ogni query.\todo{puoi citare questo \url{https://www.researchgate.net/publication/262565583_Lombardy_EPA_Obtorto_Collo_Data_and_Anti-pollution_Policies_Fallacies} in bibliografia}

\begin{figure}
    \hfill\includegraphics[width=0.7\linewidth]{img/arpa.png}\hspace*{\fill}
    \caption{Vecchio sito Arpa}
\end{figure}\todo{atrent: citare sempre fonte immagini}
...
dal 2010 al 2018.\todo{atrent: devi tentare di "dimostrare" che la tua ricerca era sensata, è difficile dimostrare di NON aver trovato qualcosa, ma almeno esplicita che ricerche hai fatto e su quali siti}
...
\'E anche necessario chiedersi quanto il numero dei patentati per regione sia uno stimatore\todo{atrent: terminologia statistica, attenzione alla semantica, verifica correttezza} 
...
oltre a informazioni sulla superficie e il perimetro delle zone.\todo{atrent: ci starebbe bene un /excerpt/ del dataset, per capire come'è fatto, vale per tutti i dataset}
...
e con esso il rischio di incidenti.\todo{atrent: tutto da dimostrare, io te lo contesterei subito, secondo me semplicemente il numero maggiore di veicoli che transitano alza la statistica}
...
considerazione i mezzi pubblici. \todo{atrent: al contario, io ad esempio evito come la peste le strade con i mezzi pubblici specie se so che possono bloccarmi (es. via Torino)}
...
strada}.\todo{atrent: non è professionale fare ipotesi poco fondate}
...
atrent: inteso come un capitolo "conclusioni"? ci può stare, breve, in cui riassumi i problemi affrontati e le informazioni raccolte/calcolate
aggiungo: importante sarebbe anche (sempre nelle conclusioni) fare un rapporto sullo stato dei dati disponibili, sulla loro qualità e granularità (btw mi viene in mente che servirebbe una digressione, in zona intro, sugli opendata) e sulle migliorie che si potrebbero chiedere ai produttori degli opendata che hai man mano trovato}
