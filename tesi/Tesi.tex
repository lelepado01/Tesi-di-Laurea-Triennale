...
\def\lighttext#1{{\color{LightTextColor}#1}}
...
\noindent\lighttext{\texttt{(\\
    ...
    \indent areac : campo booleano che indica se l'area C sia attiva o meno\\
)}} 
...
\noindent\lighttext{\texttt{(\\
    MUNICIPIO : id della zona,\\
    AREA,\\
    PERIMETRO,\\
    geometry : insieme di punti che formano il poligono\\
)}} 
...
\noindent\lighttext{\texttt{(\\
    Date\_Time: campo che contiene giorno, mese e anno,\\
    Temperature,\\
    Dew\_Point,\\
    Humidity,\\
    Wind\_Speed\\
)}}
\todo{finire, zenodo non funziona}
...
\noindent\lighttext{\texttt{(\\
    linea,\\
    mezzo : bus, filobus o tram,\\
    percorso : identificatore percorso,\\
    verso, nome,\\
    tipo\_perc : Canonico o Barrato,\\
    lung\_km, num\_ferm,\\
    geometry : insieme di linee che formano i percorsi\\
)}}
